...
define( 'WCPAY_SUBSCRIPTIONS_ABSPATH', WC_ABSPATH . 'packages/woocommerce-subscriptions-core/' );

if ( ! function_exists( 'wcpay_register_autoloader' ) ) {
	/**
	 * Register the WooPayments autoloader for core usage.
	 */
	function wcpay_register_autoloader() {
		$prefixes = array(
			'WCPay\\MultiCurrency\\'        => WCPAY_ABSPATH . 'includes/multi-currency/',
			'WCPay\\PaymentMethods\\Configs\\' => WCPAY_ABSPATH . 'includes/payment-methods/Configs/',
			'WCPay\\Vendor\\'               => WCPAY_ABSPATH . 'lib/packages/',
			'WCPay\\'                       => WCPAY_ABSPATH . 'src/',
		);

		spl_autoload_register(
			static function ( string $class ) use ( $prefixes ) {
...
add_action( 'woocommerce_newly_installed', 'wcpay_activated' );
